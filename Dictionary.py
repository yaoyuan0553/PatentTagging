...
from Utils import padSentences
...
class TagDictionary(object):
    ...
    def convertToInputTensor(self, sents: List[List[str]], device: torch.device,
        ...
        sentsIdxs = torch.tensor(padSentences(idxs, self['[PAD]']), dtype=torch.long, device=device)
        if batchFirst:
            return sentsIdxs
        return torch.t(sentsIdxs)

    def save(self, filepath: str):
        json.dump(list(self.idx2word.values())[1:], open(filepath, 'w'))
